...
const SingleUserOrder = (props) => {
    ...
    const dateFormat = (dateObject) => {
        ...
        const newDate = //date formated => dd/mm/yyyy
                ...
                <Grid item sm={12} xs={11} justify='flex-start' alignItems='flex-start'>
                    <Grid item>
                        <List
                            ...
                            subheader={
                                <ListSubheader>
                                    <Typography color='textPrimary'>Zamówienienie {props.id}:</Typography>{' '}
                                </ListSubheader>
                            }
                        >
                                ...
                                return (
                                    <>
                                        <ListItem>
                                            <ListItemText
                                                primary={item.name}
                                                secondary={`Ilość: ${item.quantity} Cena: ${item.price}`}
                                            />
                                        </ListItem>
                                        <Divider />
                                    </>
                            ...
                            <ListItem>
                                <Grid container item direction='row' justify='space-between'>
                                    <Typography>
                                        Łączna wartość: {props.value.productsValue + props.value.shipmentValue}
                                    </Typography>
